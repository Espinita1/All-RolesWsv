...
$jsonFile = $null                                                         # Json file content
...
$dirXML = Get-ChildItem .\imports\DeploymentConfigTemplate-dns-dhcp-create.xml    # Get the XML full path directory                                                                                                      # Get the XML full path directory
...
$xmlPath = $null                                                          # Get XML full path name
$svName = $null                                                           # Server Name to be changed in future updates
$newIpAddress = $null                                                     # Default Script IP address to be changed in future updates
$dnsScope = $null                                                         # Get the ddns scope from json file
$setDomainName = $null                                                    # Domain Name
$linePSComputerName = '<S N="PSComputerName">'+$svName+"</S>"             # Replacement line for <PSComputer Name Attribute in XML file> to be changed in future updates          # Replacement line for <PSComputer Name Attribute in XML file> to be changed in future updates
$lineSvName = '<S N="ServerName">'+$svName+"</S>"                         # Replacement line for <Server Name Attribute in XML file> to be changed in future updates          # Replacement line for <Server Name Attribute in XML file> to be changed in future updates
...
function Get-Params{
    Param (
        ...
        #Set Domain Name
        [Parameter(Mandatory = $true, HelpMessage="Enter a name for your Domain" )]
        [ValidateNotNullOrEmpty()]
        [string]$ADDomainName
    )
}
...
<#          
    This function can be modified for future purposes maybe, it can be reused for another script
...
#Gave it his own function 21.7.22  
function CreateDedicatedAnnuaryPartition{
...
#Gave it his own function 21.7.22  
function ModifyNetAdapterParameters  {
...
function UnregisterTask {
    ...
    Write-Debug "Continuing script from restart" | Out-File -FilePath .\log.txt -Append
    #Register the task, why not add this in the function?
    #It won't work, variables are erased after restarting or the end of the script therefore
    #It is better to register the scheduled task and erase it after the restart
    #Gave it his own function 21.7.22  
    $taskIsRegistered = Get-ScheduledTask -TaskName $taskName

    Write-Debug "Erasing scheduled task" | Out-File -FilePath .\log.txt -Append

    #To later unregister it so it doesn't run each time at logon IF it exists
    if($taskIsRegistered)
    {
...
function SetVars {
    ...
    $setDomainName = $jsonFile.setDomainName
    #The disk size we want for the AD database is 20 GB that we'll get from the maximum space storage
    $diskNewSize = $diskSize - $desiredDiskSize
}
...
#################### MAIN #####################
<# At computer name change restart phase do initialize vars and ask for input
   Find a way to skip this when the XML file exists#>
if()


<# At DHCP DNS & AD installed end phase #>
